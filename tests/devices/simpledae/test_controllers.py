import pytest
from ophyd_async.core import get_mock_execute, get_mock_put, set_mock_value
...
async def test_period_per_point_controller_begins_run_in_setup_and_ends_in_teardown(
    ...
    get_mock_execute(simpledae.controls.end_run).assert_called_once()
...
async def test_aborting_period_per_point_controller_aborts_in_teardown(
    ...
    get_mock_execute(simpledae.controls.abort_run).assert_called_once()
...
async def test_period_per_point_controller_changes_periods_and_counts(
    ...
    get_mock_execute(simpledae.controls.resume_run).assert_called_once()
    get_mock_put(simpledae.period_num).assert_called_once_with(1)

    set_mock_value(simpledae.run_state, RunstateEnum.PAUSED)
    await period_per_point_controller.stop_counting(simpledae)
    get_mock_execute(simpledae.controls.pause_run).assert_called_once()
...
async def test_run_per_point_controller_starts_and_ends_runs(
    ...
    get_mock_execute(simpledae.controls.begin_run).assert_called_once()
