...
### This library ({py:obj}`ibex_bluesky_core`)

In a clean installation of IBEX, {py:obj}`ibex_bluesky_core` is installed as a python module via pip into the
{external+ibex_developers_manual:doc}`uktena <system_components/python/Building-and-installing-uktena>` environment which is in `c:\instrument\apps\python3`.

However, for instruments which have been testing bluesky and may have versions of {py:obj}`ibex_bluesky_core` with local
modifications, a version of {py:obj}`ibex_bluesky_core` may be checked out into `c:\instrument\dev\ibex_bluesky_core` on the
instrument, and this will have been editable-installed into the {external+ibex_developers_manual:doc}`uktena <system_components/python/Building-and-installing-uktena>` environment which is in `c:\instrument\apps\python3` environment.

### Other bluesky libraries ({py:obj}`ophyd_async`, {py:obj}`bluesky`, ...)

{py:obj}`ophyd_async` and {py:obj}`bluesky` are installed as python modules. via pip, into the
{external+ibex_developers_manual:doc}`uktena <system_components/python/Building-and-installing-uktena>` environment which is in `c:\instrument\apps\python3`, because
they are specified in `pyproject.toml` of {py:obj}`ibex_bluesky_core`, which itself is a dependency of uktena.
...
The default log level is {py:obj}`logging.INFO` and all messages from {py:obj}`ibex_bluesky_core`, {py:obj}`bluesky` and {py:obj}`ophyd_async` are captured.

If you need to increase this to {py:obj}`logging.DEBUG` to isolate an issue, you can do so using 
...
To run a plan, you need to pass it to the {py:obj}`RE <bluesky.run_engine.RunEngine>` object, which is made for you by default in the IBEX GUI. It is an instance of the bluesky {py:obj}`~bluesky.run_engine.RunEngine` created by {py:obj}`~ibex_bluesky_core.run_engine.get_run_engine`.
...
:::{admonition} Non-interactive usage
If you need to run a plan non-interactively, use {py:obj}`ibex_bluesky_core.run_engine.run_plan` rather than embedding an `RE` call in a script. Note that this will cause keyboard-interrupt handling to fall back to a simpler mechanism; rather than pausing the scan to allow for it to be manually aborted or resumed, it will always abort the scan.
:::
...
attempting further operations with the {py:obj}`~bluesky.run_engine.RunEngine`.

- {py:func}`RE.resume() <bluesky.run_engine.RunEngine.resume>` - resumes the scan from the point at which it was interrupted. The plan will complete as usual.
- {py:func}`RE.stop() <bluesky.run_engine.RunEngine.stop>` and {py:func}`RE.abort() <bluesky.run_engine.RunEngine.abort>` are functionally identical - the plan will gracefully terminate, including running any
cleanup actions and calling registered callbacks.
- {py:func}`RE.halt() <bluesky.run_engine.RunEngine.halt>` tells the RunEngine to drop dead and not do **anything** on the way out. This includes cleanup handlers,
which for example may return motors to sensible positions or return DAE configuration to a state in which data can be
taken.

If you are unsure exactly what state your {py:obj}`~bluesky.run_engine.RunEngine` is in, `RE.state` can be used to check this. If you attempt to 
...
A plan will return a {py:obj}`~bluesky.run_engine.RunEngineResult` object, which has a `plan_result` attribute:
...
[It is not advised to hide the `RE(...)` call in a script/function](https://blueskyproject.io/bluesky/main/tutorial.html#plans-in-series).
The `RE(...)` call should be typed by the user, at the terminal. If you need to run a plan as part of a
...
Top-level plan wrappers should use the {py:obj}`~ophyd_async.plan_stubs.ensure_connected` plan stub from {py:obj}`ophyd_async`:
...
you may use a {py:obj}`~ibex_bluesky_core.devices.block.block_w` instead. This will both read and write to the same PV. Similarly, if `blockname:SP:RBV` does
...
If instead you get a {py:obj}`TypeError` that looks like:
...
This is because the **datatype of the underlying PV** does not match the **declared type in bluesky**. {py:obj}`ophyd_async`
will not allow you to connect a `block_r(str, "some_block")` if `"some_block"` is a float-type PV. Every signal in
{py:obj}`ophyd_async` is strongly typed.
...
See the options on {py:obj}`~ibex_bluesky_core.devices.block.BlockWriteConfig` for detailed options. These are specified
...
For complex cases, where a `set` being complete depends on multiple conditions, a custom {py:obj}`ophyd_async` device is usually
the cleanest way to accomplish this. These can be defined in an instrument's `inst` scripts area if they are specific
to one instrument, or can be promoted to {py:obj}`ibex_bluesky_core` if generally useful.
...
have a {py:obj}`~ibex_bluesky_core.devices.block.BlockWriteConfig` which has a `settle_time_s` 
parameter for this purpose.

If you *really* need to sleep in a plan, rather than at the device level, use `yield from bps.sleep(duration_seconds)`.
Do **not** call {py:obj}`time.sleep` in a plan - this will stall the event loop and interfere with keyboard interrupts, 
for example.
...
{py:obj}`bluesky.preprocessors.finalize_wrapper` and 
{py:obj}`bluesky.preprocessors.contingency_wrapper`.
...
For peak-like data, you may consider using {py:obj}`ibex_bluesky_core.callbacks.CentreOfMass` or {py:obj}`bluesky.callbacks.fitting.PeakStats` instead of performing a fit - these are more robust in the presence of poor-quality data. However, these methods may not always be appropriate, depending on the type of data you need to fit.
...
plan or device doing I/O or other synchronous operations directly, rather than via {py:obj}`asyncio` or `yield from`. 

Find the offending function and replace it with it's bluesky-native equivalent. Python's {py:obj}`asyncio` module has a debug 
...
Functions declared as `async def` are {external+python:doc}`python **coroutines** <library/asyncio-task>`.

In a bluesky context, if you see an `async def` function, it is likely that you are looking at either
an {py:obj}`ophyd_async` device class, or a {py:obj}`~bluesky.run_engine.RunEngine` message handler.
...
In an `async def` function, do not perform blocking operations. For example, always use {py:obj}`await asyncio.sleep() <asyncio.sleep>`,
and not {py:func}`time.sleep()`. Similarly, do not use synchronous functions which perform I/O from a coroutine (e.g. calling
{py:obj}`genie` APIs) - use the `async` APIs provided by {py:obj}`ophyd_async` instead.
...
{external+bluesky:doc}`bluesky plan <plans>`.
...
can stall the {py:obj}`~bluesky.run_engine.RunEngine`'s internal event loop, break keyboard-interrupts, and break rewindability. Use bluesky-native
functionality instead - for example:
- {py:obj}`yield from bps.sleep() <bluesky.plan_stubs.sleep>` instead of {py:func}`time.sleep`
- {py:obj}`yield from bps.mv(block_object, value) <bluesky.plan_stubs.mv>` instead of {py:func}`g.cset(block_name, value) <genie.cset>`
- {py:obj}`value = yield from bps.rd(block_object) <bluesky.plan_stubs.rd>` instead of {py:func}`g.cget(block_name, value) <genie.cget>`
