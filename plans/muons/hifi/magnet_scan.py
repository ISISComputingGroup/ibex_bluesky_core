...
from ophyd_async.core import StandardReadable, HintedSignal, SignalR, AsyncStatus, observe_value, SignalW
...
from ibex_bluesky_core.devices.block import BlockR, block_r
...
def magnet_scan(
    ...
    *,
    ...
    yield from ensure_connected(magnet, magnetometer_x1, magnetometer_x2, magnetometer_y1, magnetometer_y2, magnetometer_z1, magnetometer_z2, fluxgate_x, fluxgate_y, fluxgate_z, psu_x, psu_y, psu_z)
    ...
    fits = [
        LiveFit(
            model.fit(), y=r, x=magnet.name  # type: ignore
        ) for r in readbacks
    ]
    
    fitplots = [LiveFitPlot(fit, ax=ax) for fit in fits]
    
    fitloggers = [LiveFitLogger(fit, r, magnet.name, READABLE_FILE_OUTPUT_DIR, postfix=r) for fit, r in zip(fits, readbacks)]

    @bpp.subs_decorator(
        [
            HumanReadableFileCallback(READABLE_FILE_OUTPUT_DIR, fields),
            LiveTable(fields),
        ] + plots + fits + fitplots + fitloggers
    )
    def _inner():
        if rel:
            plan = bp.rel_scan
            ...
            plan = bp.scan
        yield from plan([magnetometer_x1, magnetometer_x2, magnetometer_y1, magnetometer_y2, magnetometer_z1, magnetometer_z2, fluxgate_x, fluxgate_y, fluxgate_z, psu_x, psu_y, psu_z], magnet, start, stop, num=count)
