...
Specialised plans are provided in the {py:obj}`ibex_bluesky_core.plans.reflectometry` module, which wrap step scans and
adaptive scans over reflectometry parameters.

## {py:obj}`~ibex_bluesky_core.plans.reflectometry.refl_scan`

This plan is similar to the general {py:obj}`~ibex_bluesky_core.plans.scan` plan, but assumes that the single
...
As above, but wrapping {py:obj}`~ibex_bluesky_core.plans.adaptive_scan`, in order to perform a scan with dynamic
